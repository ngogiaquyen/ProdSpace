...
interface PageComponent extends ComponentItem {
  instanceId: number;
}
...
const BuilderPage = () => {
  ...
  const handleDragStartCanvas = (e: React.DragEvent<HTMLDivElement>, index: number) => {
    e.dataTransfer.setData('componentIndex', index.toString());
  };

  const handleDropCanvas = (e: React.DragEvent<HTMLDivElement>, dropIndex: number) => {
    e.preventDefault();
    if (isPreviewMode) return;
    const dragIndex = parseInt(e.dataTransfer.getData('componentIndex'), 10);
